...
run4 introduced per-(sub-)technique recall (FIX B/B2) but ran it against **mdecrevoisier/EVTX-to-MITRE-Attack**, which is foldered at **bare-parent** technique (`TAxxxx/Txxxx`) only. The asymmetric matcher scores a SUB-technique-tagged rule (e.g. `attack.t1059.001`) ONLY against an **exact** `T1059.001` corpus bucket — and mdecrevoisier has none. So every sub-technique-tagged rule came back **`unmeasured`**: no recall credit, the run4 gap this run closes.

run5 keeps the recall math identical and swaps in a **sub-technique-labeled** recall corpus from **splunk/attack_data** (Apache-2.0), foldered `datasets/attack_techniques/<TECH>/<source>/windows-sysmon.log` where the folder name IS the ATT&CK (sub-)technique (e.g. `T1059.001`). The Sysmon **line-XML** logs (one `<Event>` per line, no single root) are converted (`scripts/build_attack_data_corpus.py`) into wrapped `<Events>...</Events>` XML documents Zircolite ingests via `--xml-input`; each fired event's `OriginalLogfile` (the .xml basename) joins to its sub-technique via the file_technique map — the SAME recall join as run4, keyed on `_stable_event_id` (fix C). **Precision (the benign corpus) is byte-for-byte UNCHANGED.**

Recall scoping is unchanged from FIX B2 (asymmetric, no sibling dilution): a `T1059.001` rule is measured against `T1059.001` events ONLY (never its `T1059.003` siblings); a bare-parent rule covers all its children. Rules with no usable tag, or whose tags match zero corpus events, stay **`unmeasured`** (NOT 0, NOT pooled).

- **rule -> technique coverage:** 555/609 loaded rules carry a usable `attack.tXXXX` tag.
- **recall-measurable rules:** 338/609 (technique present in the corpus); **unmeasured:** 271.

### Corpus provenance (honestly disclosed)

**splunk/attack_data** (Apache-2.0). Only the sub-technique folders matching a loaded rule's `attack.tXXXX.xxx` tag were pulled (via the Git-LFS media API — git-lfs was not installed for a sparse-checkout), so a few MB-to-GB of relevant Sysmon logs, not the full ~9 GB repo. Each `windows-sysmon.log` is **line-XML** (`Microsoft-Windows-Sysmon/Operational`, EID-1). Converted to wrapped-XML and run via `--xml-input` (the same streaming flattener as native EVTX, so `OriginalLogfile` is set per event exactly as the run4 join expects).

- sub-technique corpus: **24644** process-creation (EID-1) events across **46** distinct **sub-techniques** (46 sub-technique + 0 bare-parent buckets). **97** wrapped-XML files mapped to a sub-technique; **8** source logs skipped (no EID-1 event — excluded, NOT silently counted).
- technique skew (HONEST): the corpus is dominated by `T1053.005`=4282, `T1059.001`=4051, `T1048.003`=3468, `T1566.001`=1626, `T1087.002`=909 — recall for low-population sub-techniques rests on very few events.
- event identity: per-event sub-technique is keyed on `_stable_event_id` (fix C); a fire is joined to its sub-technique via `OriginalLogfile` (the .xml basename) -> sub-technique (`scripts/build_attack_data_corpus.py`, `data/comiset/attack_data_technique_map.json`).
...
_run hash (worker-invariant): `7e6821cb40f138824c599c0e85aa6f342e51032f4439e146b73d99bbe1538eb0`_
...
|---|---|---|---|---|---|---|---|
| Powershell Token Obfuscation - Process Creation | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potential Windows Defender Tampering Via Wmic.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious ClickFix/FileFix Execution Pattern | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Ping Hex IP | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Child Process Of SQL Server | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Remote Access Tool - Renamed MeshAgent Execution - Windows | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| PowerShell Download and Execution Cradles | 0.00910179640718563 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious PowerShell Parent Process | 0.0009874105159219946 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Remote CHM File Download/Execution Via HH.EXE | 0.010101010101010102 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| HackTool - SysmonEOP Execution | unmeasured | unmeasured | 0 | 0 | 15092 | 15092 | n/a (unmeasured) |
| ETW Trace Evasion Activity | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious Speech Runtime Binary Child Process | 0.0006983240223463687 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| CobaltStrike Load by Rundll32 | 0.011976047904191617 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Potential RDP Tunneling Via SSH | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potential Meterpreter/CobaltStrike Activity | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Potential RDP Tunneling Via Plink | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| PowerShell Base64 Encoded FromBase64String Cmdlet | 0.002221673660824488 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Change Default File Association To Executable Via Assoc | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Boot Configuration Tampering Via Bcdedit.EXE | unmeasured | 1.0 | 34 | 0 | 17124 | 16962 | real |
...
| System Restore Registry Modification via CommandLine | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious File Encoded To Base64 Via Certutil.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious RDP Redirect Using TSCON | 1.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| VolumeShadowCopy Symlink Creation Via Mklink | 0.0014326647564469914 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Visual Basic Command Line Compiler Usage | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Odbcconf.EXE Suspicious DLL Location | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| PUA - Ngrok Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Malicious PowerShell Commandlets - ProcessCreation | 0.007151370679380214 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Imports Registry Key From an ADS | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Remotely Hosted HTA File Executed Via Mshta.EXE | 0.0038314176245210726 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| File Decoded From Base64/Hex Via Certutil.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious File Downloaded From Direct IP Via Certutil.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potential Defense Evasion Activity Via Emoji Usage In CommandLine - 4 | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious FileFix Execution Pattern | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| HackTool - Empire PowerShell Launch Parameters | 0.0004937052579609973 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious JavaScript Execution Via Mshta.EXE | 0.007662835249042145 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| PrintBrm ZIP Creation of Extraction | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| All Backups Deleted Via Wbadmin.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| MMC20 Lateral Movement | 0.02066115702479339 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Shadow Copies Deletion Using Operating Systems Utilities | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Command Patterns In Scheduled Task Creation | 0.00023353573096683791 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Program Names | 0.021796407185628742 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| ShimCache Flush | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Persistence Via Sticky Key Backdoor | 0.007751937984496124 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| PUA - DefenderCheck Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| File Download with Headless Browser | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| HackTool - Covenant PowerShell Launcher | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspect Svchost Activity | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Potential Data Stealing Via Chromium Headless Debugging | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| SQLite Firefox Profile Data DB Access | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious UltraVNC Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| RestrictedAdminMode Registry Value Tampering - ProcCreation | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Copy From VolumeShadowCopy Via Cmd.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Bypass UAC via CMSTP | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Control Panel Items | 0.002150537634408602 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Malicious Base64 Encoded PowerShell Keywords in Command Lines | 0.0007405578869414959 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Potential LethalHTA Technique Execution | 0.02681992337164751 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious PowerShell IEX Execution Patterns | 0.0029622315477659837 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Webshell Tool Reconnaissance Activity | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Regsvr32 Execution From Highly Suspicious Location | 0.05660377358490566 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Encoded PowerShell Command Line | 0.0017279684028634905 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Registry Modification From ADS Via Regini.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| CMSTP Execution Process Creation | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious Ping/Del Command Combination | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Base64 Encoded PowerShell Command Detected | 0.0014811157738829918 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Sensitive File Access Via Volume Shadow Copy Backup | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious IIS Module Registration | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| PowerShell Base64 Encoded Invoke Keyword | 0.001974821031843989 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potentially Suspicious Child Process Of Regsvr32 | 0.05660377358490566 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious PowerShell Download and Execute Pattern | 0.00740557886941496 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious Uninstall of Windows Defender Feature via PowerShell | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| CreateDump Process Dump | 0.004464285714285714 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Enable LM Hash Storage - ProcCreation | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Set Suspicious Files as System Files Using Attrib.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| HKTL - SharpSuccessor Privilege Escalation Tool Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potential Arbitrary File Download Using Office Application | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious New Service Creation | 0.017361111111111112 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious Service Path Modification | 0.001736111111111111 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Script Event Consumer Spawning Process | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| HTML Help HH.EXE Suspicious Child Process | 0.0015734515806036333 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious PowerShell Encoded Command Patterns | 0.0007405578869414959 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| HackTool - Rubeus Execution | 0.0016611295681063123 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Debugger Registration Cmdline | 0.06976744186046512 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious Redirection to Local Admin Share | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Dumping of Sensitive Hives Via Reg.EXE | 0.008695652173913044 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| TrustedPath UAC Bypass Pattern | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Uninstall Crowdstrike Falcon Sensor | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Renamed Office Binary Execution | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Reg Add Suspicious Paths | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Deletion of Volume Shadow Copies via WMI with PowerShell | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Calculator Usage | 0.001440922190201729 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potential Rundll32 Execution With DLL Stored In ADS | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Explorer Process with Whitespace Padding - ClickFix/FileFix | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Schtasks Schedule Types | 0.0009341429238673517 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| User Added To Highly Privileged Group | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| RunMRU Registry Key Deletion | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| HackTool - Hydra Password Bruteforce Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Potentially Suspicious Execution From Parent Process In Public Folder | 0.0009580838323353293 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Potential Powershell ReverseShell Connection | 0.00024685262898049864 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Python One-Liners with Base64 Decoding | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious File Downloaded From File-Sharing Website Via Certutil.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious Binary In User Directory Spawned From Office Application | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| HackTool - PPID Spoofing SelectMyParent Tool Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| HackTool - SharpImpersonation Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious HH.EXE Execution | 0.0012873694750393362 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| RDP Port Forwarding Rule Added Via Netsh.EXE | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| MMC Spawning Windows Shell | 0.0371900826446281 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| HackTool - Mimikatz Execution | 0.005188067444876783 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Uncommon One Time Only Scheduled Task At 00:00 | 0.00023353573096683791 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Schtasks Creation Or Modification With SYSTEM Privileges | 0.0009341429238673517 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potential Reconnaissance For Cached Credentials Via Cmdkey.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Potential Defense Evasion Via Rename Of Highly Relevant Binaries | 0.004322766570605188 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potential Tampering With RDP Related Registry Keys Via Reg.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Scheduled Task Executing Encoded Payload from Registry | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Process Memory Dump Via Comsvcs.DLL | 0.0017857142857142857 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| File Explorer Folder Opened Using Explorer Folder Shortcut Via Shell | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Net WebClient Casing Anomalies | 0.0014811157738829918 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potential PowerShell Command Line Obfuscation | 0.0014811157738829918 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Fsutil Suspicious Invocation | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious Process Created Via Wmic.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| LOL-Binary Copied From System Directory | 0.004322766570605188 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| PowerShell Base64 Encoded IEX Cmdlet | 0.0012342631449024932 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| HackTool - Bloodhound/Sharphound Execution | 0.002554060956921505 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| HackTool - SharpView Execution | 0.015873015873015872 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potential Privilege Escalation Using Symlink Between Osk and Cmd | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| HackTool - SharpDPAPI Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Process Execution From A Potentially Suspicious Folder | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious Process Patterns NTDS.DIT Exfil | 0.0056657223796034 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Rar Usage with Password and Compression Level | 0.0056022408963585435 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious MSHTA Child Process | 0.12643678160919541 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Mavinject Inject DLL Into Running Process | 0.16666666666666666 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Copying Sensitive Files with Credential Data | 0.0028653295128939827 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Delete All Scheduled Tasks | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Security Event Logging Disabled via MiniNt Registry Key - Process | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| HackTool - SharpUp PrivEsc Tool Execution | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| MSHTA Execution with Suspicious File Extensions | 0.007662835249042145 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| System File Execution Location Anomaly | 0.008645533141210375 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Run PowerShell Script from ADS | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Tor Client/Browser Execution | 0.12167300380228137 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| UAC Bypass Using Windows Media Player - Process | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Sysmon Driver Unloaded Via Fltmc.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| HackTool - Hashcat Password Cracker Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Child Process Of Wermgr.EXE | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Bypass UAC via Fodhelper.exe | 0.003067484662576687 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Explorer NOUACCHECK Flag | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potential LSASS Process Dump Via Procdump | 0.007142857142857143 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Abused Debug Privilege by Arbitrary Parent Processes | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| LSASS Dump Keyword In CommandLine | 0.051643192488262914 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| PUA - AdvancedRun Suspicious Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| PUA - CleanWipe Execution | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Potentially Suspicious Event Viewer Child Process | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Uncommon Svchost Command Line Parameter | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious Rundll32 Execution With Image Extension | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
| Suspicious PowerShell Parameter Substring | 0.017032831399654405 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| File In Suspicious Location Encoded To Base64 Via Certutil.EXE | unmeasured | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| New User Created Via Net.EXE With Never Expire Option | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
| Suspicious Process Parents | 0.0 | unmeasured | 0 | 0 | 17124 | 16962 | n/a (unmeasured) |
...
|---|---|---|---|
| Suspicious RDP Redirect Using TSCON | T1563.002 | 1.0000 | 5/5 |
| Renamed Mavinject.EXE Execution | T1055.001,T1218.013 | 0.2500 | 3/12 |
| Mavinject Inject DLL Into Running Process | T1055.001,T1218.013 | 0.1667 | 2/12 |
| Suspicious MSHTA Child Process | T1218.005 | 0.1264 | 33/261 |
| Tor Client/Browser Execution | T1090.003 | 0.1217 | 32/263 |
| Suspicious Debugger Registration Cmdline | T1546.008 | 0.0698 | 9/129 |
| Regsvr32 Execution From Highly Suspicious Location | T1218.010 | 0.0566 | 6/106 |
| Potentially Suspicious Child Process Of Regsvr32 | T1218.010 | 0.0566 | 6/106 |
| LSASS Dump Keyword In CommandLine | T1003.001 | 0.0516 | 22/426 |
| HackTool - Potential Impacket Lateral Movement Activity | T1021.003 | 0.0372 | 9/242 |
| MMC Spawning Windows Shell | T1021.003 | 0.0372 | 9/242 |
| Potential LethalHTA Technique Execution | T1218.005 | 0.0268 | 7/261 |
| Suspicious Program Names | T1059.001,T1059.003,T1059.005 | 0.0218 | 91/4175 |
| MMC20 Lateral Movement | T1021.003 | 0.0207 | 5/242 |
| Suspicious New Service Creation | T1543.003 | 0.0174 | 10/576 |
| Suspicious PowerShell Parameter Substring | T1059.001 | 0.0170 | 69/4051 |
| HackTool - SharpView Execution | T1069.002 | 0.0159 | 2/126 |
| CobaltStrike Load by Rundll32 | T1218.011 | 0.0120 | 6/501 |
| Remote CHM File Download/Execution Via HH.EXE | T1218.001 | 0.0101 | 1/99 |
| PowerShell Download and Execution Cradles | T1059.001,T1059.003,T1059.005 | 0.0091 | 38/4175 |
| Dumping of Sensitive Hives Via Reg.EXE | T1003.002 | 0.0087 | 3/345 |
| System File Execution Location Anomaly | T1036.003 | 0.0086 | 6/694 |
| Persistence Via Sticky Key Backdoor | T1546.008 | 0.0078 | 1/129 |
| Suspicious JavaScript Execution Via Mshta.EXE | T1218.005 | 0.0077 | 2/261 |
| MSHTA Execution with Suspicious File Extensions | T1218.005 | 0.0077 | 2/261 |
| Suspicious PowerShell Download and Execute Pattern | T1059.001 | 0.0074 | 30/4051 |
| Malicious PowerShell Commandlets - ProcessCreation | T1059.001,T1069.001,T1069.002,T1087.001,T1087.002 | 0.0072 | 42/5873 |
| Potential LSASS Process Dump Via Procdump | T1003.001,T1036.003 | 0.0071 | 8/1120 |
| HackTool - Windows Credential Editor (WCE) Execution | T1003.001 | 0.0070 | 3/426 |
| Suspicious Process Patterns NTDS.DIT Exfil | T1003.003 | 0.0057 | 2/353 |
| Rar Usage with Password and Compression Level | T1560.001 | 0.0056 | 2/357 |
| HackTool - Mimikatz Execution | T1003.001,T1003.002 | 0.0052 | 4/771 |
| CreateDump Process Dump | T1003.001,T1036.003 | 0.0045 | 5/1120 |
| Potential Defense Evasion Via Rename Of Highly Relevant Binaries | T1036.003 | 0.0043 | 3/694 |
| LOL-Binary Copied From System Directory | T1036.003 | 0.0043 | 3/694 |
| Windows Shell/Scripting Processes Spawning Suspicious Programs | T1059.001,T1059.005,T1218.001,T1218.002,T1218.005,T1218.007,T1218.010,T1218.011,T1218.013 | 0.0040 | 21/5242 |
| Remotely Hosted HTA File Executed Via Mshta.EXE | T1218.005 | 0.0038 | 1/261 |
| UAC Bypass Using ChangePK and SLUI | T1548.002 | 0.0031 | 2/652 |
| Bypass UAC via Fodhelper.exe | T1548.002 | 0.0031 | 2/652 |
| Suspicious PowerShell IEX Execution Patterns | T1059.001 | 0.0030 | 12/4051 |
| Copying Sensitive Files with Credential Data | T1003.002,T1003.003 | 0.0029 | 2/698 |
| HackTool - Bloodhound/Sharphound Execution | T1059.001,T1069.001,T1069.002,T1087.001,T1087.002 | 0.0026 | 15/5873 |
| Hacktool Execution - Imphash | T1003.001,T1003.002,T1003.003,T1588.002 | 0.0024 | 3/1237 |
| PowerShell Base64 Encoded FromBase64String Cmdlet | T1059.001 | 0.0022 | 9/4051 |
| Control Panel Items | T1218.002,T1546.003,T1546.008,T1546.015 | 0.0022 | 2/930 |
| Bad Opsec Defaults Sacrificial Processes With Improper Arguments | T1218.011 | 0.0020 | 1/501 |
| PowerShell Base64 Encoded Invoke Keyword | T1059.001 | 0.0020 | 8/4051 |
| Process Memory Dump Via Comsvcs.DLL | T1003.001,T1036.003 | 0.0018 | 2/1120 |
| Suspicious Service Path Modification | T1543.003 | 0.0017 | 1/576 |
| Suspicious Encoded PowerShell Command Line | T1059.001 | 0.0017 | 7/4051 |
| HackTool - Rubeus Execution | T1003.001,T1003.002,T1003.003,T1550.003,T1558.003 | 0.0017 | 3/1806 |
| HTML Help HH.EXE Suspicious Child Process | T1059.001,T1059.003,T1059.005,T1218.001,T1218.002,T1218.005,T1218.007,T1218.010,T1218.011,T1218.013,T1566.001 | 0.0016 | 11/6991 |
| Base64 Encoded PowerShell Command Detected | T1059.001 | 0.0015 | 6/4051 |
| Net WebClient Casing Anomalies | T1059.001 | 0.0015 | 6/4051 |
| Potential PowerShell Command Line Obfuscation | T1059.001 | 0.0015 | 6/4051 |
| Suspicious Calculator Usage | T1036.003 | 0.0014 | 1/694 |
| VolumeShadowCopy Symlink Creation Via Mklink | T1003.002,T1003.003 | 0.0014 | 1/698 |
| Suspicious HH.EXE Execution | T1059.001,T1059.003,T1059.005,T1218.001,T1218.002,T1218.005,T1218.007,T1218.010,T1218.011,T1218.013,T1566.001 | 0.0013 | 9/6991 |
| PowerShell Base64 Encoded IEX Cmdlet | T1059.001 | 0.0012 | 5/4051 |
| Suspicious PowerShell Parent Process | T1059.001 | 0.0010 | 4/4051 |
| Potentially Suspicious Execution From Parent Process In Public Folder | T1059.001,T1059.003,T1059.005 | 0.0010 | 4/4175 |
| Suspicious Schtasks Schedule Types | T1053.005 | 0.0009 | 4/4282 |
| Schtasks Creation Or Modification With SYSTEM Privileges | T1053.005 | 0.0009 | 4/4282 |
| Malicious Base64 Encoded PowerShell Keywords in Command Lines | T1059.001 | 0.0007 | 3/4051 |
| Suspicious PowerShell Encoded Command Patterns | T1059.001 | 0.0007 | 3/4051 |
| Suspicious Speech Runtime Binary Child Process | T1021.003,T1218.001,T1218.002,T1218.005,T1218.007,T1218.010,T1218.011,T1218.013 | 0.0007 | 1/1432 |
| HackTool - Empire PowerShell Launch Parameters | T1059.001 | 0.0005 | 2/4051 |
| Potential Powershell ReverseShell Connection | T1059.001 | 0.0002 | 1/4051 |
| Suspicious Command Patterns In Scheduled Task Creation | T1053.005 | 0.0002 | 1/4282 |
| Uncommon One Time Only Scheduled Task At 00:00 | T1053.005 | 0.0002 | 1/4282 |
