...
> **Benign corpus composition (A8 honesty):** 17124 Sysmon EID-1 events = ~2032 from COMISET REAL (a real university network, incl. 162 malicious-labelled) + ~15092 NextronSystems/evtx-baseline goodware (Apache-2.0, benign-by-construction). ~88% is synthetic goodware, NOT university traffic — hence `precision@combined-benign`, not a pure `precision@COMISET`. Per-event provenance was not retained this run, so a rule's FPs cannot be split COMISET-vs-Nextron; adding a `sigmaforge_origin` tag is the next-run fix.
...
> **Recall caveat (POOLED DENOMINATOR — do not read per-rule recall at face value):** recall here = tp / 1533, where 1533 = ALL process-creation attack events across all techniques. But each rule targets ONE technique with only a handful of matching events, so a rule that perfectly catches its technique still shows recall ≈ 0.001–0.016. **The recall column is near-zero by construction and is NOT a per-rule detection rate.** This is the recall-side twin of the precision tautology; the fix (per-technique recall = tp / events-of-that-technique) is scheduled. Until then, do not present these recall numbers externally.
