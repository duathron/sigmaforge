...
- [ ] **Step 3 — create `sigmaforge/pipeline.py`.** A frozen dataclass `PipelineConfig` (NOTE: distinct from the user-facing `config.BacktestConfig` added in C2.1 — `PipelineConfig` is the pipeline's resolved input, `config.BacktestConfig` is the YAML/flag config the CLI resolves INTO a `PipelineConfig`) with the fields run7 hardcodes: `compiled_ruleset: str`, `loaded_rules_glob: str`, `attack_corpus: str`, `technique_map: str`, `benign_sample: str`, `comiset_mapping: str`, `evtx_cfg: str`, `min_events: int = 1000`, `source: str = "COMISET"`, plus optional `report_extras: dict` (run-specific provenance passed as DATA, e.g. run6 slice info / path-form note). A `BacktestResult` dataclass that exposes everything a caller needs to rebuild a run-specific header WITHOUT inlining prose into the pipeline: `rows`, `funnel` (carries `recall_mode`), `manifest: dict`, `report_md: str`, `run_hash: str`, PLUS the COMPUTED provenance run7's header embeds today — `acceptance_gate` (the attack/benign `engine_fires`/`scored_fires`/`dropped_titles`/`ok` rows from `assert_one_source`), `n_attack_events`, `technique_event_counts`, `rules_recall_measurable`, `rules_precision_measurable`, `loaded_rule_count`. `run_backtest_pipeline(cfg: PipelineConfig)` = the lifted body of `run7_backtest.main` with the module constants replaced by `cfg.*`. The pipeline stays corpus-AGNOSTIC: it builds the GENERIC report body + the above computed values, and does NOT compute or embed run7/OpTC-specific prose. Run-SPECIFIC caveats (e.g. run7's OpTC Image path-form split, which is computed from the benign corpus) are computed by the CALLER (run7_backtest.run()) from the corpus it owns and injected via `render_report`'s `caveats` param (Task 2.4). Return a `BacktestResult`.
...
- [ ] **Step 1 — failing test.** Assert each row carries `recall_reason` / `precision_reason` (None when measured, the code when unmeasured), and that `funnel["recall_mode"]` is `"per-technique"` when technique inputs present and `"pooled"` when absent. Add to the existing `test_orchestrate.py` patterns (which already unpack `rows, funnel, md`).
- [ ] **Step 2 — run, expect FAIL.**
- [ ] **Step 3 — implement.** In `run_backtest`, where a row's recall is set, also set `row["recall_reason"] = per_technique_recall_reason(...)`; where precision is set, `row["precision_reason"] = precision_reason(...)`. Compute `recall_mode` (`"per-technique"` if `event_technique`+`technique_event_counts` given, else `"pooled"`) and carry it on the `funnel` dict: `funnel["recall_mode"] = ...`. **DO NOT change `run_backtest`'s 3-tuple return arity** — `(rows, funnel, md)` is unpacked at 18 sites (`scripts/run2-7_backtest.py`, `run_first_backtest.py`, `sigmaforge/main.py`, 10× in `tests/test_orchestrate.py`); a 4th return value breaks run4/5/6 and the C1 oracle. Keep existing numeric outputs identical (re-run the C1 gate after).
...
- [ ] **Step 2 — run, expect FAIL.**
- [ ] **Step 3 — implement** `hunt(rules, logs, out, config)`: preflight engine (exit 3); `compile_ruleset(paths)` (Task 3.1) returns an IN-MEMORY `list[dict]` but `run_shard(events_path, ruleset_glob, ...)` needs a ruleset FILE path → serialize the compiled list to a `tempfile.NamedTemporaryFile(suffix=".json")` and pass THAT path to `run_shard` over the logs (set `json_input`/`xml_input` by detecting EVTX vs JSON; no corpus mapping needed) → collect hits (rule_id, event_id, count). Write `hits.json` with a top-level `"precision": "unmeasured — unlabeled corpus"` + `"recall": "unmeasured — unlabeled corpus"` and the banner; print the banner to stdout. Exit 0.
- [ ] **Step 4 — run, expect PASS.**
