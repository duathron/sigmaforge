...
def main() -> int:
    ...
    header = (
        ...
        "run4 measures each rule against **only the attack events of its own ATT&CK technique(s)**: "
        "`recall = (unique events of the rule's technique it fired on) / (total events of that "
        "technique)`. Sub-techniques fold to parent (T1543.003 -> T1543). Rules with no technique "
        "tag, or whose technique has zero attack events in the corpus, are **`unmeasured`** "
        "(NOT 0, NOT pooled).\n\n"
        ...
        f"**{tmap['techniques_with_pc']}** techniques; **{tmap['files_mapped']}** EVTX files "
        f"mapped to a technique, **{len(tmap['files_unmappable'])}** unmappable "
        "(non-`TAxxxx`-foldered helper dirs — excluded, NOT silently counted).\n"
        f"- technique skew (HONEST): the corpus is dominated by "
        + ", ".join(f"`{t}`={c}" for t, c in top_tech)
        + " — recall for low-population techniques rests on very few events.\n"
